...
async function fetchTrending() {
  ...
  try {
    ...
    movies.forEach((item, i) => {
      ...
      c.innerHTML = `<div class="card-rank">${i + 1}</div><div class="w-full max-h-full aspect-[5/8] overflow-hidden" style="border-radius:8px;position:relative"><figure class="m-0 leading-[0]"><picture class="block w-full h-full" style="display:block!important">${poster ? `<img src="${poster}" alt="${title}" width="170" height="272" loading="lazy" fetchpriority="auto" onerror="this.style.display='none'" style="width:100%;height:100%;object-fit:cover;display:block" />` : ''}</picture></figure><div class="card-play"><svg viewBox="0 0 24 24"><polygon points="8,5 19,12 8,19" fill="#fff"/></svg></div><div class="card-img-overlay"></div><div class="card-info"><div class="card-rating">&#9733; ${rating}</div><h4>${title}</h4><p>${type}${year ? ` - ${year}` : ''}</p></div></div>`;
      trk.appendChild(c);
    });
