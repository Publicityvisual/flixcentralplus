...
async function fetchTrending() {
  ...
  try {
    ...
    movies.forEach((item, i) => {
      ...
      c.innerHTML = `<div class="card-rank">${i + 1}</div><div class="w-full max-h-full aspect-[5/8] overflow-hidden" style="border-radius:8px;position:relative;background:${colors[i]}"><figure class="m-0 leading-[0]" style="position:absolute;inset:0;z-index:1"><picture class="inline-block align-top w-full h-full" style="display:inline-block!important;vertical-align:top;width:100%;height:100%">${poster ? `<source srcset="${poster}" media="(max-width: 768px)"><source srcset="${poster.replace('w342','w500')}" media="(min-width: 769px)"><img src="${poster}" alt="${title}" width="170" height="272" loading="lazy" fetchpriority="auto" onerror="this.parentElement.style.backgroundColor='${colors[i]}'" class="w-full max-w-full" style="width:100%;height:100%;object-fit:cover;display:block" />` : ''}</picture></figure><div class="card-overlay"></div></div>`;
      trk.appendChild(c);
    });
