...
async function fetchTrending() {
  ...
  try {
    ...
    movies.forEach((item, i) => {
      ...
      c.innerHTML = `<div class="card-img" style="background:${colors[i]}">${poster ? `<img src="${poster}" alt="${title}" loading="lazy" width="180" height="270" onerror="this.style.display='none'" />` : ''}<div class="card-play"><svg viewBox="0 0 24 24"><polygon points="8,5 19,12 8,19" fill="#fff"/></svg></div><div class="card-img-overlay"></div><div class="card-info"><div class="card-rating">&#9733; ${rating}</div><h4>${title}</h4><p>${type}${year ? ` - ${year}` : ''}</p></div></div><div class="card-rank">${i + 1}</div>`;
      trk.appendChild(c);
    });
