...
async function fetchTrending() {
  ...
  try {
    ...
    movies.forEach((item, i) => {
      ...
      c.innerHTML = `<div class="card-img" style="background:${colors[i]}">${poster ? `<img src="${poster}" alt="${title}" loading="lazy" width="180" height="270" />` : ''}<div class="card-img-overlay"></div><div class="card-rank">${i + 1}</div><div class="card-info"><div class="card-rating">&#9733; ${rating}</div><h4>${title}</h4><p>${type}${year ? ` - ${year}` : ''}</p></div></div>`;
      trk.appendChild(c);
    });
    ...
    popular.forEach(item => {
      ...
      c.innerHTML = `<div class="card-img" style="background:#1a1a1a">${poster ? `<img src="${poster}" alt="${title}" loading="lazy" width="140" height="210" />` : ''}<div class="card-img-overlay"></div><div class="card-info"><div class="card-rating">&#9733; ${rating}</div><h4>${title}</h4><p>${TR[lang].tr.mov}${year ? ` - ${year}` : ''}</p></div></div>`;
      trk.appendChild(c);
    });
  } catch (e) {}
}
